...
SKIP_DATA = [""]
SAVE_DIR = os.path.join(cfg.eval_root, "test_inf_pc")
...
def process_one(path):
    ...
    gt_pc_path = os.path.join(cfg.eval_root, "test_gt_pc/")+ data_id + ".ply"
    if not os.path.exists(gt_pc_path):
        print("no gt pc:", data_id)
        return None
...
def run(args):
    ...
    # all step under cfg.result_path
    filepaths = sorted(glob.glob(os.path.join("../"+cfg.result_path, "*.step")))    # remove ../ when debug
    if args.num != -1:
        filepaths = filepaths[:args.num]

    save_path = cfg.eval_root + '/test_cd_pc_stat.txt'
    record_res = None
