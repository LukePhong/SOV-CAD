...
def _get_padding_mask(commands, seq_dim=0, extended=False):
    with torch.no_grad():
        ...
        if extended:
            ...
            # Create a new tensor of the same shape
            result = padding_mask.clone()
            # Sum adjacent elements (without in-place modification)
            result[:, 1:] = padding_mask[:, 1:] + padding_mask[:, :-1]
            # Clamp the result to 1
            padding_mask = result.clamp(max=1)
