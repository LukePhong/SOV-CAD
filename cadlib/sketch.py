...
class Loop(SketchBase):
    ...
    @staticmethod
    def from_dict(stat):
        curve_key = "profile_curves" if "profile_curves" in stat else "curves"
        all_curves = [construct_curve_from_dict(item) for item in stat[curve_key]]   #NOTE: profile_curves
    ...
    def draw_image(self, fig, ax, img=[], start_point=False, line_width=2, line_color='black'):
        # colors = ['red', 'blue', 'green', 'brown', 'pink', 'yellow', 'purple', 'black'] * 10
        # draw a start point
        if start_point and (isinstance(self.children[-1], Line) or isinstance(self.children[-1], Arc)):
            xdata = [self.end_point[0], self.end_point[0]]
            ydata = [self.end_point[1], self.end_point[1]]
            l1 = lines.Line2D(xdata, ydata, lw=line_width, color=line_color, axes=ax)
            ax.add_line(l1)
            ax.plot(self.end_point[0], self.end_point[1], 'o', color=line_color, markersize=max(3, line_width*2))
            fig.canvas.draw()
            img_array = np.array(fig.canvas.renderer.buffer_rgba())[:, :, :3]
            img.append(deepcopy(img_array))
        for i, curve in enumerate(self.children):
            curve.draw_image(fig, ax, line_color, img, line_width)
...
class Profile(SketchBase):
    ...
    def draw_image(self, fig, ax, img=[], start_point=False, line_width=2, line_color='black'):
        for i, loop in enumerate(self.children):
            # for the same op, all images should be in the same list
            loop.draw_image(fig, ax, img, start_point, line_width, line_color)
